---
title: " "
layout: splash
permalink: /team/
header:
  overlay_color: "#FFF"
  overlay_filter: "0"
  overlay_image: /assets/splash/brain3a.jpg
Manuel:
  - image_path: /assets/team/2026_9_Manuel.jpg
    title: "Manuel Schottdorf, Ph.D."
    excerpt: "Manuel studied physics before completing a Ph.D. at Max Planck in Germany and a Postdoc at the Princeton Neuroscience Institute. He enjoys developing scientific instrumentation and won numerous awards over the years, among them a Boehringer Ingelheim Fonds PhD Fellowship, an Otto Hahn medal, and a Burroughs Wellcome Fund's Career Award. In addition to science, Manuel's interests include labor rights and he was an organizing committee member for the postdoc union at Princeton.&nbsp;&nbsp;&nbsp;&nbsp;Send Manuel an [e-mail](mailto:maschott-at-udel.edu)."
Mubariz:
  - image_path: /assets/team/2025_7_Mubariz.jpg
    title: "Mubariz Mohammed"
    excerpt: "Mubariz is a second year PhD student in the lab. His research broadly focuses on visual processing and decision making in the mouse visual cortex. In the lab, he performs survival surgeries, including cranial window implantation and viral injections, and trains mice on virtual reality visual decision making tasks such as the Towers task. He also enjoys the technical side of neuroscience, including building, aligning, and working with two-photon microscopes, as well as developing and maintaining databases for behavioral and imaging experiments. He previously earned a Master’s degree in Computational Data Science from the University of California, Riverside. Outside the lab, Mubariz enjoys hiking and camping, exploring new cities and cuisines, and learning about different cultures."
Ethan:
  - image_path: /assets/team/2026_9_Ethan.jpg
    title: "Ethan Castanon"
    excerpt: "Ethan Castanon is a Psychological & Brain Sciences Ph.D. student in the Schottdorf Lab with a foundation in psychology. His prior work focused on ocular behavior and eye-tracking in humans, which shaped his current focus on how animals explore and map their environments using a systems neuroscience approach. Aside from Ethan’s research, he loves to draw, play the electric guitar, and video games."
Gayathri:
  - image_path: /assets/team/2026_8_Gayathri.jpg
    title: "Gayathri Kotti"
    excerpt: "Gayathri is an undergraduate student studying neuroscience and physics at the University of Delaware. In the Schottdorf Lab, she is currently working on computational approaches to perception and generalization, including analyzing behavioral confusion matrices and modeling pairwise similarity structures. Her current interests include how psychological distance can be inferred from observed behavior and how these quantitative models can help explain perception and learning. Outside of research, Gayathri is an Indian classical dancer and loves to read classic literature."  
Ryan:
  - image_path: /assets/team/2026_8_Ryan.jpg
    title: "Ryan Kirstein"
    excerpt: "Ryan Kirstein is an undergraduate in the Schottdorf lab, majoring in psychology with a minor in Human Development and Family Sciences. Ryan is currently working with the mice to get them to vocalize at ultrasonic frequencies by themselves. Besides research Ryan plays rugby for the University of Delaware"  
---
# PI
{% include feature_row id="Manuel" type="left" %}

# The team
{% include feature_row id="Mubariz" type="left" %}
{% include feature_row id="Ethan" type="left" %}
{% include feature_row id="Gayathri" type="left" %}
{% include feature_row id="Ryan" type="left" %}

# Alums
{: style="text-align: left; font-size:1.5em;"}
Dr. SuHyeong Kim, Ph.D. Now with Blackrock Neurotech as a Translational R&D Engineer.
Kathleen Higgins, Now undergrad at Georgetown as a Goldwater Scholar.
Lindsey Wang, Now graduate student at Northwestern U.

<!-- {% include feature_row %} -->

